...
export const ExpensesProvider = ({ children }: { children: ReactNode }) => {
  ...
  const add = async (groupId: string, localData: ExpenseInput) => {
  ...
  const update = async (expenseId: string, localData: ExpenseInput) => {
    setItems((prev) =>
  ...
  const set = (newItems: Expense[]) => setItems(newItems);
  ...
  return (
    <ExpensesContext.Provider value={{ items, add, update, set, remove }}>
      {children}
